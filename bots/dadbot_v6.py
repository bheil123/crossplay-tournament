...
Architecture (inherited from v5):
  ...
  - Main process: aggregate results, add leave value + positional adj
...
_gaddag_obj = None  # Cached GADDAG object (needed for 3-ply)

def _ensure_resources():
    global _gdata_bytes, _word_set, _gaddag_obj
    if _gdata_bytes is not None:
        return
    from engine.gaddag import get_gaddag
    _gaddag_obj = get_gaddag()
    _gdata_bytes = bytes(_gaddag_obj._data)
