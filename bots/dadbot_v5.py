...
Performance tiers (BOT_TIER env var):
  ...
  - standard: ~3s/move   (same as fast -- higher N/K regresses)
  - deep:     ~3s/move   (same as fast -- higher N/K regresses)

v5 change: A/B testing showed fast tier (N=15, K=400) outperforms
higher-compute tiers against a constant-strength opponent. Standard
(N=30, K=1500) and deep (N=35, K=2000) both regressed vs fast,
indicating systematic MC evaluation bias amplified by more compute.
All tiers now cap at fast-tier parameters until the root cause is fixed.
