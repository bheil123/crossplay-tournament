...
# Heuristic disable flag for A/B testing (env: DADBOT_DISABLE_HEURISTIC)
# Values: "risk", "blocking", "dls", "dd", "turnover", "hvt", or empty
_DISABLE_HEURISTIC = os.environ.get('DADBOT_DISABLE_HEURISTIC', '').lower()
...
def _compute_positional_adj(grid, move, unseen_pool, bag_size, real_risk_mode=False):
    ...
    # Zero out disabled heuristic for A/B testing
    if _DISABLE_HEURISTIC == 'risk':
        risk = 0.0
    elif _DISABLE_HEURISTIC == 'blocking':
        blocking = 0.0
    elif _DISABLE_HEURISTIC == 'dls':
        dls = 0.0
    elif _DISABLE_HEURISTIC == 'dd':
        dd = 0.0
    elif _DISABLE_HEURISTIC == 'turnover':
        turnover = 0.0
    elif _DISABLE_HEURISTIC == 'hvt':
        hvt = 0.0
...
class DadBot(BaseEngine):
    ...
    def __init__(self):
        ...
        self.config = dict(TIERS.get(tier, TIERS['fast']))  # Copy for overrides
        self.tier = tier

        # Environment variable overrides for A/B testing
        # e.g. DADBOT_OVERRIDE_POSITIONAL_ADJ=false
        _bool_overrides = [
            'POSITIONAL_ADJ', 'EXCHANGE_EVAL', 'BLANK_3PLY',
            'REAL_RISK', 'EXCHANGE_MC',
        ]
        for key in _bool_overrides:
            env_val = os.environ.get(f'DADBOT_OVERRIDE_{key}')
            if env_val is not None:
                self.config[key] = env_val.lower() in ('true', '1', 'yes')
                print(f"  [DadBot] Override: {key}={self.config[key]}")
    ...
    def pick_move(self, board, rack, moves, game_info):
        ...
        use_pos_adj = cfg.get('POSITIONAL_ADJ', True)
        pos_adjs = []
        ...
        if exch_futures:
            ...
            if exch_leave > best_1ply_equity and best_total < 0:
                print(f"  [DadBot] EXCHANGE: dump={exch_opts[0].get('dump', '?')} "
                      f"keep={exch_opts[0].get('keep', '')} "
                      f"(exp_leave={exch_leave:.1f} > best_1ply={best_1ply_equity:.1f}, "
                      f"mc_total={best_total:.1f}), bag={bag_tiles}")
                return None
            elif exch_opts:
                print(f"  [DadBot] EXCHANGE considered but rejected: "
                      f"exp_leave={exch_leave:.1f}, best_1ply={best_1ply_equity:.1f}, "
                      f"mc_total={best_total:.1f}, bag={bag_tiles}")
        elif not exch_opts and cfg.get('EXCHANGE_EVAL', True) and bag_tiles >= RACK_SIZE:
            # Exchange was skipped because best_1ply >= threshold
            pass  # Normal -- best play is strong enough
    ...
    def _blank_3ply_check(self, board, rack, candidates_with_results,
        ...
        print(f"  [DadBot] BLANK_3PLY triggered: {blanks_in_rack} blanks "
              f"in rack, bag={bag_size}, MC winner={best_move['word']} "
              f"({best_move['score']}pts, eq={best_total:.1f})")
        ...
        if best_3ply is not None and best_3ply_eq > best_total + 2.0:
            print(f"  [DadBot] BLANK_3PLY OVERRIDE: {best_3ply['word']} "
                  f"({best_3ply['score']}pts, 3ply_eq={best_3ply_eq:.1f}) "
                  f"beats MC winner eq={best_total:.1f} by "
                  f"{best_3ply_eq - best_total:.1f}")
            return best_3ply, best_3ply_eq

        if best_3ply is not None:
            print(f"  [DadBot] BLANK_3PLY no override: best_3ply={best_3ply['word']} "
                  f"(3ply_eq={best_3ply_eq:.1f}) vs MC winner "
                  f"eq={best_total:.1f}, gap={best_3ply_eq - best_total:.1f}")
        else:
            print("  [DadBot] BLANK_3PLY: no valid 3-ply results")
        return best_move, best_total
